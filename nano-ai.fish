...
# set model "gemini-2.5-pro-preview-05-06"
...
duckdb (status dirname)/nano-ai.db \
    -c "create table if not exists logs (created_at timestamp default current_timestamp, prompt varchar, response json);" \
    -c "insert into logs (prompt, response) values(\$\$"$prompt"\$\$, \$\$"$response"\$\$);" &
